...
class AttentionModel(nn.Module):
    ...
    def build_projection_network(self):
        ...
        W1 = torch.randn(IMAGE_EMBEDDING_SIZE, SEMANTIC_EMBEDDING_SIZE, requires_grad = True).to(self.device)
        W2 = torch.randn(IMAGE_EMBEDDING_SIZE, SEMANTIC_EMBEDDING_SIZE, requires_grad = True).to(self.device)
        W3 = torch.randn(IMAGE_EMBEDDING_SIZE, SEMANTIC_EMBEDDING_SIZE, requires_grad = True).to(self.device)
        W4 = torch.randn(IMAGE_EMBEDDING_SIZE, SEMANTIC_EMBEDDING_SIZE, requires_grad = True).to(self.device)
    ...
    def forward(self, x, eval_mode=False):
        ...
        for k in self.crop_networks:
            ...
            projection = torch.mm(crop_embedding, projection_matrix)
            
            projections.append(reduction)
