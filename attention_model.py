...
class AttentionModel(nn.Module):
    ...
    def load_model(device, name):
        ...
        for k in model.keys:
            ...
            model.dimension_reduction_networks[k].load_state_dict(torch.load(name + '/' + 'dimension_reduction_networks_' +k))
        
        model.global_dimension_reduction.load_state_dict(torch.load(name + '/' + 'global_dimension_reduction'))
        model.output.load_state_dict(torch.load(name + '/output'))
    ...
    def __init__(self, device, load_pretrained_networks = False):
        ...
        self.dimension_reduction_networks = self.build_reduction_reduction_network()
        
        self.global_dimension_reduction = self.build_global_dimension_reduction()
        
        self.output = nn.Linear(REDUCTION_SIZE * 4,1).to(self.device)
    ...
    def save_model(self, name):
        ...
        for k in self.feature_map_attention_network:
            ...
            torch.save(self.dimension_reduction_networks[k].state_dict(), name + '/' + 'dimension_reduction_networks_' +k)
        
        torch.save(self.global_dimension_reduction.state_dict(), name + '/' + 'global_dimension_reduction')
        torch.save(self.output.state_dict(), name + '/output')
    ...
    def eval_mode(self):
        ...
        self.global_dimension_reduction.eval()
        self.output.eval()
    ...
    def train_mode(self):
        ...
        self.global_dimension_reduction.train()
        self.output.train()
        

    def build_global_dimension_reduction(self):
        return Sequential(
            nn.Linear(FEATURE_MAPS, 256),
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(256, 128),
            nn.ReLU(),
    ...
    def build_reduction_reduction_network(self):
        reduction1 = Sequential(
            nn.Linear(FEATURE_MAPS, 256),
            nn.ReLU(),
            nn.Dropout(0.4),
            nn.Linear(256, 128),
        ...
        reduction_network = {}
        reduction_network['att1'] = reduction1
        reduction_network['att2'] = reduction2
        reduction_network['att3'] = reduction3
        reduction_network['att4'] = reduction4
    ...
    def get_model_parameters(self):
        ...
        for k in self.feature_map_attention_network:
            ...
            parameters += list(self.dimension_reduction_networks[k].parameters())
            
        parameters += list(self.global_dimension_reduction.parameters())
    ...
    def forward(self, x, eval_mode=False):
        ...
        multi_part_reductions = []
        for k in self.crop_networks:
            crop_network = self.crop_networks[k]
            attention_maps_part = attention_maps[k]
            boxes = crop_network(attention_maps_part.view(attention_maps_part.shape[0],-1))
            crops = self.take_crops(boxes, x)
            reduction_network = self.dimension_reduction_networks[k]
            ...
            reduction = reduction_network(crop_embeddings)
            
            multi_part_reductions.append(reduction)
            
        global_features = self.global_dimension_reduction(feature_map_avg_pool.view(feature_map_avg_pool.shape[0],-1))
            
        #multi_part_reductions.append(global_features)
        
        features = torch.cat(multi_part_reductions, dim=1)
        
        out = self.output(features)
        
        return torch.sigmoid(out), multi_attention_loss